    ...
    nivelActual(numNivel)
{
    ...
    player->setZValue(2);
    entidades.append(player);     // Se registra como Entidad (Jugador)
...
// -------------------------------------------------------------------
//  Mensaje de bloqueo de ataque (nivel 1)
// -------------------------------------------------------------------
...
// -------------------------------------------------------------------
//  Actualización general de la escena (Tick del juego)
// -------------------------------------------------------------------
void niveles::actualizarEscena()
{
    ...
    // ----------------------------------------------------------
    // NIVEL 2: barco que se mueve horizontal + proyectiles
    // ----------------------------------------------------------
    if (nivelActual == 2) {
    ...
    // ----------------------------------------------------------
    // OTROS NIVELES (1 y 3): se usa polimorfismo con Entidad
    // ----------------------------------------------------------
    ...
    // Limitar movimiento horizontal del mapa (nivel 1 largo)
    if (centro.x() < 0)
        player->setPos(0, 600);
    if (centro.x() > 3700)
    ...
    // ----------------------------------------------------------
    // NIVEL 3: colisión física directa con enemigos
    // ----------------------------------------------------------
    if (nivelActual == 3) {
    ...
    // ----------------------------------------------------------
    // NIVEL 1: sigilo, centinela detecta al jugador
    // ----------------------------------------------------------
    if (nivelActual == 1) {
    ...
    // ----------------------------------------------------------
    // ATAQUE DEL JUGADOR (niveles 2 y 3 usan combate, 1 no)
    // ----------------------------------------------------------
    if (player->atacando) {
...
// -------------------------------------------------------------------
//  Configuración base de la escena (fondos, tamaño, monedas, HUD)
// -------------------------------------------------------------------
...
void niveles::actualizarProyectilesNivel2()
{
    ...
    if (tiempoNivel2 % 120 == 0) {     // cada 2 segundos aprox
    ...
    for (int i = proyectiles.size() - 1; i >= 0; i--) {
        ...
        if (boxJugador.intersects(p->sceneBoundingRect())) {
            ...
            // Knockback lateral pequeño (usando colisiones físicas simples)
            Cuerpo J, B;
            J.masa = 4.0f;
